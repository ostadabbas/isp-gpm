...
function defineD_n_layers(input_nc, output_nc, ndf, n_layers)
    if n_layers==0 then
        ...
        for n = 1, n_layers-1 do 
            nf_mult_prev = nf_mult  -- nf mult increase from 1, 2, 4, to at most 8 8 x64 = 512  fts
            nf_mult = math.min(2^n,8)
            netD:add(nn.SpatialConvolution(ndf * nf_mult_prev, ndf * nf_mult, 4, 4, 2, 2, 1, 1))    -- k s p ((w+2p-k)/dw + 1 = (w+2-4)/2 +1 = w/2
        ...
        netD:add(nn.Sigmoid())
        -- state size: 1 x (N-2) x (N-2)    a heat map
