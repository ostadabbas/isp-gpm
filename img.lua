...
-- add rot in there, no compatible with the rot operation
function transformPreds(coords, center, scale, rot, res)
    -- multiple scales transform 
    local origDims = coords:size()
    coords = coords:view(-1,2)    -- two columns
    local newCoords = coords:clone()
    for i = 1,coords:size(1) do
        newCoords[i] = transform(coords[i], center, scale, rot, res, 1)
    end
...
function crop(img, center, scale, rot, res, method)
    ...
    if scaleFactor < 2 then scaleFactor = 1
        ...
        if newSize < 2 then
           ...
           return newImg
            ...
            tmpImg = image.scale(img,newSize, method)   -- maximum width or height
    ...
    local ul = transform({1,1}, c, s, 0, res, true)  -- no rotation coord
    ...
    local new_ = {1,-1,math.max(1, -ul[2] + 2), math.min(br[2], ht+1) - ul[2],
                       math.max(1, -ul[1] + 2), math.min(br[1], wd+1) - ul[1]}  -- why -ul[2]+2 ??
